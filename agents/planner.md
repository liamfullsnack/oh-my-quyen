...
**IMPORTANT**: Analyze the list of skills at `.qwen/skills/*` and intelligently activate the skills that are needed for the task during the process.
...
```bash
node .qwen/scripts/set-active-plan.cjs {plan-dir}
```

Example:
```bash
node .qwen/scripts/set-active-plan.cjs ai_docs/feature/GH-88-add-authentication
```
