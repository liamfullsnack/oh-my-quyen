...
**IMPORTANT**: Analyze skills at `.qwen/skills/*` and activate as needed.

## Gemini Model Configuration

Read model from `.qwen/.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
[ ! -f .gemini/settings.json ] && mkdir -p .gemini && ln -sf .qwen/.mcp.json .gemini/settings.json
...
```bash
npx tsx .qwen/skills/mcp-management/scripts/cli.ts call-tool <server> <tool> '<json-args>'
```
