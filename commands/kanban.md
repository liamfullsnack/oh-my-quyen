...
The user invoked **/kanban** with arguments (e.g. [dir]): {{args}}
...
**IMPORTANT:** Run server as Qwen Code background task using `run_in_background: true` with the Bash tool. This makes the server visible in `/tasks` and manageable via `KillShell`.

The skill is located at `.qwen/skills/plans-kanban/`.
...
```bash
node .qwen/skills/plans-kanban/scripts/server.cjs --stop
```
...
node .qwen/skills/plans-kanban/scripts/server.cjs \
  --dir "$PLANS_DIR" \
  --host 0.0.0.0 \
  --open \
  --foreground
...
```json
{
  "command": "node .qwen/skills/plans-kanban/scripts/server.cjs --dir \"./plans\" --host 0.0.0.0 --open --foreground",
...
Track progress: https://github.com/oh-my-quyen/oh-my-quyen/issues/189
